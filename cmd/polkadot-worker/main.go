...
import (
	...
	storeHTTP "github.com/figment-networks/indexing-engine/worker/store/transport/http"
...
func createIndexerClient(ctx context.Context, log *zap.Logger, cfg *config.Config, conns *proxy.GRPConnections, connApi *api.Conn, ds *scale.DecodeStorage) *indexer.Client {
	...
	httpStore := storeHTTP.NewHTTPStore(storeEndpoints, nil, http.DefaultClient)
	...
	return indexer.NewClient(log, proxyClient, ds, httpStore, connApi, cfg.Exp, uint64(cfg.MaximumHeightsToGet), cfg.ChainID, cfg.Currency, cfg.Network)
}
